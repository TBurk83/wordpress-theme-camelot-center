...
<?php if(have_posts()): while(have_posts()): the_post();?>
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h5>
              <em>
                <span class="text-muted author"><?php _e('By', 'camelot'); echo " "; the_author() ?>,</span>
        ...
        <section>
            ...
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'camelot' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'camelot' ) ); ?>
        </section>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="fa fa-folder-open-o"></i>&nbsp; <?php _e('Category', 'camelot'); ?>: <?php the_category(', ') ?><br/>
                <i class="fa fa-comment-o"></i>&nbsp; <?php _e('Comments', 'camelot'); ?>: <?php comments_popup_link(__('None', 'camelot'), '1', '%'); ?>
            </p>
        </footer>
    </article>
<?php endwhile; ?>

<?php if ( function_exists('camelot_pagination') ) { camelot_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="fa fa-arrow-left"></i> ' . __('Previous', 'camelot')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'camelot') . ' <i class="fa fa-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
